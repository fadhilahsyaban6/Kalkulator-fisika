...
UNIT_TABLE = {
    "Panjang": {"m": 1, "km": 1000, "cm": 0.01, "mm": 0.001, "mum": 1e-6, "nm": 1e-9, "ft": 0.3048, "in": 0.0254, "mi": 1609.34},
...
def calc_density(mass, volume, mass_unit, vol_unit):
    ...
    if vol_unit == "L" or vol_unit == "dm^3":
        vol_m3 = volume * 0.001
    elif vol_unit == "mL" or vol_unit == "cm^3":
        vol_m3 = volume * 1e-6
    elif vol_unit == "ft^3":
...
def calc_viscosity(force, area, velocity, gradient, f_unit, a_unit, v_unit, g_unit):
    # eta = (F/A) / (dv/dy) = shear stress / shear rate
...
QUIZ_QUESTIONS = [
    ...
    },
    {
        ...
        "jawaban": "20",
        "pembahasan": "Menggunakan v^2 = 2gh -> v = sqrt(2*10*20) = sqrt400 = 20 m/s"
    },
    {
        "type": "pilihan_ganda",
    ...
    {
        "type": "isian",
        "soal": "Berapa gaya gravitasi antara dua benda bermassa 1000 kg dan 2000 kg yang berjarak 10 m? (G = 6.67*10-^1^1, tulis dalam bentuk desimal dengan 4 angka di belakang koma)",
        "jawaban": "0.0013",
        "pembahasan": "F = G(m_1m_2)/r^2 = 6.67*10-^1^1 * (1000*2000)/100 = 1.334*10-^6 N. Jika dibulatkan: 0.0000013 N"
    },
    {
        "type": "pilihan_ganda",
...
# ==================== SIDEBAR NAVIGASI ====================
st.sidebar.markdown("<h1 style='text-align:center;'>(atom)️ Smart Physics</h1>", unsafe_allow_html=True)
st.sidebar.markdown("---")

menu = st.sidebar.radio(
    "(clipboard) Menu Navigasi",
    ["(home) Beranda", "(books) Rumus & Cheat Sheet", "(calculator) Kalkulator", "(counterclockwise arrows button) Unit Converter", "(quiz) Quiz Fisika"]
)

# ==================== HALAMAN BERANDA ====================
if menu == "(home) Beranda":
    st.markdown("<div class='main-header'>(atom)️ Smart Physics Calculator</div>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class='feature-card'>
            <div class='feature-icon'>(calculator)</div>
    ...
    with col2:
        st.markdown("""
        <div class='feature-card'>
            <div class='feature-icon'>(counterclockwise arrows button)</div>
    ...
    st.markdown("<div class='sub-header'>(microscope) Pengenalan Materi Fisika Dasar</div>", unsafe_allow_html=True)

    tab1, tab2, tab3, tab4 = st.tabs(["(triangular ruler) Mekanika", "(thermometer)️ Termodinamika", "(energy) Listrik & Magnet", "(speaker) Gelombang"])
    ...
    st.markdown("---", unsafe_allow_html=True)
    st.info("(idea) **Tips**: Gunakan menu di sidebar untuk mengakses fitur kalkulator, konverter, dan quiz!")

# ==================== HALAMAN RUMUS & CHEAT SHEET ====================
elif menu == "(books) Rumus & Cheat Sheet":
    st.markdown("<div class='main-header'>(books) Rumus & Cheat Sheet Fisika</div>", unsafe_allow_html=True)

    tab1, tab2, tab3 = st.tabs(["(clipboard) Tabel Satuan", "(input numbers) Konstanta Fisika", "(open book) Rumus Penting"])

    with tab1:
        st.markdown("<div class='sub-header'>(clipboard) Tabel Satuan SI dan Konversi</div>", unsafe_allow_html=True)
        ...
        prefix_data = [
            ...
            ["Senti (c)", "10-^2", "0,01"],
            ["Mili (m)", "10-^3", "0,001"],
            ["Mikro (mu)", "10-^6", "0,000001"],
            ["Nano (n)", "10-^9", "0,000000001"],
            ["Piko (p)", "10-^1^2", "0,000000000001"]
        ]
        st.table(prefix_data)

    with tab2:
        st.markdown("<div class='sub-header'>(input numbers) Konstanta Fisika Fundamental</div>", unsafe_allow_html=True)
    ...
    with tab3:
        st.markdown("<div class='sub-header'>(open book) Rumus-Rumus Penting Fisika Dasar</div>", unsafe_allow_html=True)
        ...
        for category, formulas in rumus_categories.items():
            with st.expander(f"(pushpin) {category}"):
...
# ==================== HALAMAN KALKULATOR ====================
elif menu == "(calculator) Kalkulator":
    st.markdown("<div class='main-header'>(calculator) Smart Physics Calculator</div>", unsafe_allow_html=True)
    ...
    calc_type = st.selectbox(
        "Pilih Kalkulator:",
        ["(bar chart) Kerapatan (Density)", "(honey) Viskositas (Kekentalan)", "(triangular ruler) Sudut Reposisi", "(energy) Persamaan Gas Ideal", "(counterclockwise arrows button) Konversi Satuan"]
    )

    # --- KALKULATOR KERAPATAN ---
    if calc_type == "(bar chart) Kerapatan (Density)":
        st.markdown("<div class='sub-header'>(bar chart) Kalkulator Kerapatan (rho = m/V)</div>", unsafe_allow_html=True)
        ...
        if st.button("(search) Hitung Kerapatan", type="primary"):
            if mass <= 0 or volume <= 0:
                st.error("(cross mark) Massa dan volume harus lebih besar dari 0!")
            else:
                # Konversi ke SI
                mass_kg = auto_convert(mass, mass_unit, "kg", "Massa")
                ...
                # Konversi volume
                vol_conversions = {
                    "m^3": mass, "L": 0.001, "cm^3": 1e-6, "mL": 1e-6, 
                ...
                st.success("(check mark button) Perhitungan Berhasil!")

                st.markdown("<div class='sub-header'>(clipboard) Langkah Pengerjaan:</div>", unsafe_allow_html=True)
                ...
                st.markdown("<div class='sub-header'>(open book) Penjelasan Rumus:</div>", unsafe_allow_html=True)
                st.info("""
                **Kerapatan (rho)** adalah ukuran massa per satuan volume suatu zat. 
                Rumus: **rho = m/V**

                - **rho** (rho) = kerapatan (kg/m^3)
                - **m** = massa benda (kg)
                - **V** = volume benda (m^3)

                **Interpretasi Fisika:**
                - Air murni: rho ~= 1000 kg/m^3 (pada 4 derajatC)
                - Udara: rho ~= 1.225 kg/m^3 (pada 15 derajatC, 1 atm)
                - Besi: rho ~= 7870 kg/m^3
                - Kerapatan relatif (specific gravity) = rho_zat / rho_air
                """)

    # --- KALKULATOR VISKOSITAS (METODE OSWALD) ---
    elif calc_type == "(honey) Viskositas (Kekentalan)":
        st.markdown("<div class='sub-header'>(honey) Kalkulator Viskositas Metode Oswald</div>", unsafe_allow_html=True)

        st.markdown("""
        <div class='formula-box' style='text-align:center;'>
            <h3>(triangular ruler) Rumus Viskositas Metode Oswald</h3>
        ...
        with col1:
            st.markdown("<h4 style='color:#1f77b4;'>(lotion bottle) Fluida Uji (misal: Air Sabun)</h4>", unsafe_allow_html=True)
        ...
        with col2:
            st.markdown("<h4 style='color:#ff7f0e;'>(droplet) Fluida Referensi (misal: Air Suling)</h4>", unsafe_allow_html=True)
            ...
            eta_ref = st.number_input("Viskositas fluida referensi (eta_1) dalam Pa*s:", min_value=0.0, value=0.001002, step=0.0001, format="%.6f", help="Air suling 20 derajatC = 0.001002 Pa*s")

        if st.button("(search) Hitung Viskositas", type="primary"):
            if t_ref <= 0 or rho_ref <= 0:
                st.error("(cross mark) Waktu alir dan densitas referensi harus lebih besar dari 0!")
            else:
                # Rumus Oswald: eta_2 = (t_2 * rho_2 * eta_1) / (t_1 * rho_1)
                ...
                <div style='background-color:#fff3cd; padding:20px; border-radius:12px; text-align:center; margin:20px 0;'>
                    <h2 style='color:#ff7f0e; margin:0;'>(honey) Viskositas Fluida Uji</h2>
                ...
                st.markdown("<div class='sub-header'>(open book) Penjelasan Teori & Rumus:</div>", unsafe_allow_html=True)
                ...
                **Referensi:**
                * Air suling 20 derajatC: eta = 0.001002 Pa*s, rho = 1000 kg/m^3
                * Air suling 25 derajatC: eta = 0.00089 Pa*s, rho = 997 kg/m^3
                """)

                st.markdown("<h4>(bar chart) Referensi Viskositas & Densitas Beberapa Zat</h4>", unsafe_allow_html=True)
                ref_data = {
                    "Zat": ["Air suling (20 derajatC)", "Air suling (25 derajatC)", "Air sabun", "Madu", "Oli SAE 30", "Glikol", "Glycerin", "Bensin", "Alkohol"],
    ...
    # --- KALKULATOR SUDUT REPOSISI ---
    elif calc_type == "(triangular ruler) Sudut Reposisi":
        st.markdown("<div class='sub-header'>(triangular ruler) Kalkulator Sudut Reposisi (theta = arctan h/r)</div>", unsafe_allow_html=True)

        st.markdown("""
        <div class='formula-box' style='text-align:center;'>
            <h3>(triangular ruler) Rumus Sudut Reposisi dari Geometri Tumpukan</h3>
        ...
        if st.button("(search) Hitung Sudut Reposisi", type="primary"):
            if h <= 0 or r <= 0:
                st.error("(cross mark) Tinggi (h) dan jari-jari (r) harus lebih besar dari 0!")
            else:
                ...
                # Hitung koefisien gesek statis (mus) dari sudut reposisi
                mu_s = tan_theta
                ...
                <div style='background-color:#e8f4f8; padding:20px; border-radius:12px; text-align:center; margin:20px 0; border-left:5px solid #1f77b4;'>
                    <h2 style='color:#1f77b4; margin:0;'>(triangular ruler) Sudut Reposisi (theta)</h2>
                ...
                * Gaya gesek maksimum: f_m_a_x = mu_s mg cos(theta)
                ...
                * Pasir kering: 30 derajat-35 derajat (h/r ~= 0.58-0.70)
                * Tepung gandum: 45 derajat-55 derajat (h/r ~= 1.0-1.43)
                * Batu bara: 35 derajat-45 derajat (h/r ~= 0.70-1.0)
                * Semen: 40 derajat-50 derajat (h/r ~= 0.84-1.19)
                """)

    # --- KALKULATOR PERSAMAAN GAS IDEAL ---
    elif calc_type == "(energy) Persamaan Gas Ideal":
        st.markdown("<div class='sub-header'>(energy) Kalkulator Persamaan Gas Ideal (PV = nRT)</div>", unsafe_allow_html=True)

        st.markdown("""
        <div class='formula-box' style='text-align:center;'>
            <h3>(triangular ruler) Persamaan Gas Ideal</h3>
        ...
        cari = st.selectbox(
            "(target) Pilih yang ingin dicari:",
            ["(blue circle) Tekanan (P)", "(green circle) Volume (V)", "(yellow circle) Jumlah Mol (n)", "(red circle) Suhu (T)"]
        )

        st.markdown("---", unsafe_allow_html=True)

        # Konstanta R dalam berbagai satuan
        R_SI = 8.314  # Pa*m^3/(mol*K) = J/(mol*K)

        # ==================== CARI TEKANAN (P) ====================
        if cari == "(blue circle) Tekanan (P)":
            st.markdown("<h4 style='color:#1f77b4;'>(blue circle) Mencari Tekanan (P = nRT/V)</h4>", unsafe_allow_html=True)
            ...
            with col2:
                ...
                T_unit = st.selectbox("Satuan suhu:", ["K (Kelvin)", " derajatC (Celcius)"])
            with col3:
            ...
            if st.button("(search) Hitung Tekanan", type="primary"):
                if n_val <= 0 or V_val <= 0:
                    st.error("(cross mark) n dan V harus lebih besar dari 0!")
                else:
                    # Konversi suhu ke Kelvin
                    if T_unit == " derajatC (Celcius)":
                        T_k = T_val + 273.15
                    else:
                        T_k = T_val

                    # Konversi volume ke m^3
                    ...
                    st.markdown(f"""
                    <div style='background-color:#e8f4f8; padding:20px; border-radius:12px; text-align:center; margin:20px 0; border-left:5px solid #1f77b4;'>
                        <h2 style='color:#1f77b4; margin:0;'>(blue circle) Tekanan (P)</h2>
                    ...
                    <div class='step-box'>
                        ...
                        * n sudah dalam mol (check)<br>
                        * T dikonversi ke Kelvin: {T_val} {T_unit.split()[0]} -> {T_k:.2f} K<br>
                        * V dikonversi ke m^3: {V_val} {V_unit} -> {V_m3:.6e} m^3
                    </div>
                    <div class='step-box'>
                    ...
                    * Volume harus dalam m^3 untuk konsistensi dengan satuan R.
                    * Gas ideal adalah gas yang mengikuti hukum ini pada tekanan rendah & suhu tinggi.
                    """)

        # ==================== CARI VOLUME (V) ====================
        elif cari == "(green circle) Volume (V)":
            st.markdown("<h4 style='color:#2ca02c;'>(green circle) Mencari Volume (V = nRT/P)</h4>", unsafe_allow_html=True)
            ...
            if st.button("(search) Hitung Volume", type="primary"):
                if n_val <= 0 or P_val <= 0:
                    st.error("(cross mark) n dan P harus lebih besar dari 0!")
                else:
                    # Konversi suhu ke Kelvin
                    if T_unit == " derajatC (Celcius)":
                        T_k = T_val + 273.15
                    ...
                    <div style='background-color:#e8f4f8; padding:20px; border-radius:12px; text-align:center; margin:20px 0; border-left:5px solid #2ca02c;'>
                        <h2 style='color:#2ca02c; margin:0;'>(green circle) Volume (V)</h2>
                        <h1 style='color:#ff7f0e; margin:10px 0;'>V = {V_hasil:.4f} {V_unit_out}</h1>
                        <p style='font-size:1.1em;'>= {V_m3:.6e} m^3 = {V_m3*1000:.4f} L = {V_m3*1e6:.2f} cm^3</p>
                    </div>
                    , unsafe_allow_html=True)
                    ...
                    * Pada STP (0 derajatC, 1 atm), 1 mol gas ideal memiliki volume 22.4 L.
                    """)

        # ==================== CARI MOL (n) ====================
        elif cari == "(yellow circle) Jumlah Mol (n)":
            st.markdown("<h4 style='color:#ff7f0e;'>(yellow circle) Mencari Jumlah Mol (n = PV/RT)</h4>", unsafe_allow_html=True)
            ...
            if st.button("(search) Hitung Jumlah Mol", type="primary"):
                if P_val <= 0 or V_val <= 0:
                    st.error("(cross mark) P dan V harus lebih besar dari 0!")
                else:
                    # Konversi suhu ke Kelvin
                    if T_unit == " derajatC (Celcius)":
                        T_k = T_val + 273.15
                    ...
                    <div style='background-color:#e8f4f8; padding:20px; border-radius:12px; text-align:center; margin:20px 0; border-left:5px solid #ff7f0e;'>
                        <h2 style='color:#ff7f0e; margin:0;'>(yellow circle) Jumlah Mol (n)</h2>
                    ...
                    <div class='step-box'>
                        ...
                        * V dikonversi ke m^3: {V_val} {V_unit} -> {V_m3:.6e} m^3<br>
                    ...
                    * 1 mol gas ideal pada STP (0 derajatC, 1 atm) memiliki volume 22.4 L.
                    * Konstanta R = 8.314 J/(mol*K) = 0.0821 L*atm/(mol*K).
                    """)

        # ==================== CARI SUHU (T) ====================
        elif cari == "(red circle) Suhu (T)":
            st.markdown("<h4 style='color:#d62728;'>(red circle) Mencari Suhu (T = PV/nR)</h4>", unsafe_allow_html=True)
            ...
            T_unit_out = st.selectbox("Satuan hasil suhu:", ["K (Kelvin)", " derajatC (Celcius)"])

            if st.button("(search) Hitung Suhu", type="primary"):
                if n_val <= 0:
                    st.error("(cross mark) n harus lebih besar dari 0!")
                else:
                    # Konversi tekanan ke Pa
                    P_conv_in = {"Pa": 1, "kPa": 1000, "atm": 101325, "bar": 100000, "mmHg": 133.322}
                    P_pa = P_val * P_conv_in[P_unit]
                    ...
                    if T_unit_out == " derajatC (Celcius)":
                        T_hasil = T_k - 273.15
                    else:
                        T_hasil = T_k
                    ...
                    <div style='background-color:#e8f4f8; padding:20px; border-radius:12px; text-align:center; margin:20px 0; border-left:5px solid #d62728;'>
                        <h2 style='color:#d62728; margin:0;'>(red circle) Suhu (T)</h2>
                        <h1 style='color:#ff7f0e; margin:10px 0;'>T = {T_hasil:.2f} {T_unit_out.split()[0]}</h1>
                        <p style='font-size:1.1em;'>= {T_k:.2f} K = {T_k-273.15:.2f}  derajatC = {T_k*9/5-459.67:.2f}  derajatF</p>
                    </div>
                    , unsafe_allow_html=True)
                    ...
                    * 0 K = -273.15 derajatC = titik nol mutlak (tidak ada energi kinetik).
                    * Suhu standar = 273.15 K = 0 derajatC = 32 derajatF.
                    """)

    # --- KONVERSI SATUAN ---
    elif calc_type == "(counterclockwise arrows button) Konversi Satuan":
        st.markdown("<div class='sub-header'>(counterclockwise arrows button) Kalkulator Konversi Satuan</div>", unsafe_allow_html=True)
        ...
        with col2:
            st.markdown("<br><br><h2 style='text-align:center;'>(right arrow)</h2>", unsafe_allow_html=True)

        with col3:
            to_unit = st.selectbox("Ke:", units)

        if st.button("(counterclockwise arrows button) Konversi", type="primary"):
            result = auto_convert(value, from_unit, to_unit, category)

            if result is not None:
                st.success(f"(check mark button) **{value} {from_unit} = {result:.6g} {to_unit}**")
                ...
                if category == "Suhu":
                    ...
                    <div class='step-box'>
                        ...
                        {value} derajat{from_unit} -> {result:.4f} derajat{to_unit}<br>
                ...
                st.error("(cross mark) Konversi tidak valid!")

# ==================== HALAMAN UNIT CONVERTER ====================
elif menu == "(counterclockwise arrows button) Unit Converter":
    st.markdown("<div class='main-header'>(counterclockwise arrows button) Auto Unit Converter</div>", unsafe_allow_html=True)
    ...
    converter_type = st.selectbox(
        ...
        ["(straight ruler) Panjang", "(balance scale) Massa", "(stopwatch)️ Waktu", "(thermometer)️ Suhu", "(wind) Tekanan", "(energy) Energi", "(plug) Daya", "(bar chart) Kerapatan", "(honey) Viskositas", "(flexed biceps) Gaya"]
    )

    category_map = {
        "(straight ruler) Panjang": "Panjang", "(balance scale) Massa": "Massa", "(stopwatch)️ Waktu": "Waktu",
        "(thermometer)️ Suhu": "Suhu", "(wind) Tekanan": "Tekanan", "(energy) Energi": "Energi",
        "(plug) Daya": "Daya", "(bar chart) Kerapatan": "Kerapatan", "(honey) Viskositas": "Viskositas", "(flexed biceps) Gaya": "Gaya"
    }
    ...
    if st.button("(rocket) Konversi Sekarang", type="primary", use_container_width=True):
        ...
        if result is not None:
            ...
            <div style='background-color:#e8f4f8; padding:20px; border-radius:15px; text-align:center; margin:20px 0;'>
                ...
                <h1 style='margin:10px 0;'>(down arrow)</h1>
                <h2 style='color:#ff7f0e; margin:0;'>= {result:.6g} {to_u}</h2>
            </div>
            """, unsafe_allow_html=True)

            with st.expander("(clipboard) Lihat Langkah Pengerjaan"):
                if cat == "Suhu":
                    st.markdown(f
                    **Rumus Konversi Suhu:**
                    ...
                    **Hasil:** {val} derajat{from_u} = {result:.4f} derajat{to_u}
                    )
                else:
            ...
            st.error("(cross mark) Konversi gagal. Periksa satuan yang dipilih.")

    st.markdown("---", unsafe_allow_html=True)
    st.markdown("<div class='sub-header'>(bar chart) Tabel Konversi Cepat</div>", unsafe_allow_html=True)
    ...
    quick_conversions = {
        ...
        "Suhu": [("0 derajatC", "32 derajatF", "273.15 K", "491.67 derajatR"), ("100 derajatC", "212 derajatF", "373.15 K", "671.67 derajatR")],
...
# ==================== HALAMAN QUIZ ====================
elif menu == "(quiz) Quiz Fisika":
    st.markdown("<div class='main-header'>(quiz) Quiz Fisika Dasar</div>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div style='background-color:#f0f2f6; padding:15px; border-radius:10px; text-align:center;'>
            <h3>(trophy) Skor: {st.session_state.quiz_score} / {len(QUIZ_QUESTIONS)}</h3>
    ...
    for idx, q in enumerate(QUIZ_QUESTIONS):
        with st.container():
            ...
            if q['type'] == "pilihan_ganda":
                ...
                if not answered:
                    if st.button(f"(check mark button) Submit Jawaban Soal {idx+1}", key=f"btn_pg_{idx}"):
                        st.session_state.quiz_answered.add(idx)
                        if user_answer == q['jawaban']:
                            st.session_state.quiz_score += 1
                            st.success("(check mark button) Benar!")
                        else:
                            st.error(f"(cross mark) Salah! Jawaban yang benar: **{q['jawaban']}**")
                        st.info(f"(open book) **Pembahasan:** {q['pembahasan']}")
                        st.rerun()
                else:
                ...
                if not answered:
                    if st.button(f"(check mark button) Submit Jawaban Soal {idx+1}", key=f"btn_isian_{idx}"):
                        if user_answer.strip():
                            ...
                            st.warning("(warning) Masukkan jawaban terlebih dahulu!")
                else:
                    st.info(f"(open book) **Pembahasan:** {q['pembahasan']}")
    ...
    # Tombol reset
    if st.button("(counterclockwise arrows button) Reset Quiz", type="secondary"):
        st.session_state.quiz_score = 0
        st.session_state.quiz_answered = set()
        st.rerun()
    ...
    if st.session_state.quiz_score == len(QUIZ_QUESTIONS):
        ...
        <div style='background-color:#d4edda; padding:20px; border-radius:15px; text-align:center; margin-top:20px;'>
            <h1 style='color:#155724;'>(party) Selamat! (party)</h1>
            <h2 style='color:#155724;'>Anda telah menjawab semua soal dengan benar!</h2>
            <p style='font-size:1.2em;'>Skor sempurna: {}/{}</p>
        </div>
        """.format(len(QUIZ_QUESTIONS, unsafe_allow_html=True), len(QUIZ_QUESTIONS, unsafe_allow_html=True), unsafe_allow_html=True), unsafe_allow_html=True)

# Footer
st.sidebar.markdown("---")
st.sidebar.markdown("<p style='text-align:center; color:#888;'>(atom)️ Smart Physics Calculator v1.0<br>Built with Streamlit</p>", unsafe_allow_html=True)
